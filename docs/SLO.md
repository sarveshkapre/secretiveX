...
- Leaving both tail knobs unset now auto-selects a sane tail envelope: `bench_slo_gate.sh` will prefer `secretive-agent --suggest-queue-wait-json` (host-aware) when available, and falls back to profile defaults (`pssh` uses 4ms with ≤3% tail, `fanout` 6ms/≤4%, `balanced` 8ms/≤5%, `low-memory` 12ms/≤7%). Override the env vars to customize.
