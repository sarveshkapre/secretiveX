...
- Agent metrics snapshots also include `queue_wait_suggested` (tail_ns/tail_ratio + profile label) so dashboards can compare observed queue-wait tails vs the recommended envelope.
- Leaving both tail knobs unset now auto-selects a sane tail envelope: `bench_slo_gate.sh` will prefer the host-aware suggestion helper (`secretive-agent --suggest-queue-wait-quiet`) and falls back to profile defaults (`pssh` uses 4ms with ≤3% tail, `fanout` 6ms/≤4%, `balanced` 8ms/≤5%, `low-memory` 12ms/≤7%). Override the env vars to customize.
...
- `secretive-agent --suggest-queue-wait [--profile ... | --config ...]` inspects the merged config plus hardware concurrency and prints a recommended `tail_ns`/`tail_ratio` pair (and matching env exports) so you can align CI thresholds with whatever profile or machine you're targeting before running the gate. Use `--suggest-queue-wait-json` for machine-readable output and `--suggest-queue-wait-quiet` to emit only `SLO_QUEUE_WAIT_*` assignments.
